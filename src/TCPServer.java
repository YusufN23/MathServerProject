...
public class TCPServer {
    ...
    public static void logClientConnection(String addr, Date when) {
        try (BufferedWriter w = new BufferedWriter(
                new FileWriter("logs/client_logs.txt", true)))
        {
    ...
    public static void logClientActivity(
            String addr, String req, String rsp, Date when
    ) {
        try (BufferedWriter w = new BufferedWriter(
                new FileWriter("logs/client_logs.txt", true)))
        {
    ...
    /** New: log disconnection + duration */
    public static void logClientDisconnection(
            String addr, Date connectTime, Date disconnectTime
    ) {
        long millis  = disconnectTime.getTime() - connectTime.getTime();
...
class ClientHandler implements Runnable {
        ...
        try {
            ...
            while ((line = in.readLine()) != null) {
                ...
                if (line.toUpperCase().startsWith("JOIN ")) {
                    ...
                    TCPServer.logClientActivity(
                            clientAddress, line, welcome, new Date()
                    );
                    continue;
                }

                // enqueue math or EXIT
                TCPServer.requestQueue.put(
                        new Request(this, line, new Date())
                );
    ...
    public String processMathRequest(String req) {
        ...
        try {
            ...
            return "Error: Non-integer operands";
        }
        ...
        switch (op) {
            ...
            case "DIV":
                return b != 0 ? Integer.toString(a / b)
                        : "Error: Division by zero";
            case "EXIT":
                return "GOODBYE " + clientName;
            default:
